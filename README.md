...
<img src="https://img.shields.io/badge/-%F0%9F%A7%A0%20MeetingMind-0f172a?style=for-the-badge&logoColor=38bdf8" alt="MeetingMind" height="48" />

<h3>Real-Time AI Meeting Assistant</h3>

<p><em>A context-aware AI co-pilot that listens, understands, and acts — so you can focus on the conversation.</em></p>

<br />

[![Live Demo](https://img.shields.io/badge/%F0%9F%9A%80%20Live%20Demo-meeting--assistant--sigma--roan.vercel.app-38bdf8?style=for-the-badge)](https://meeting-assistant-sigma-roan.vercel.app)

<br />

[![Next.js](https://img.shields.io/badge/Next.js-14-black?style=flat-square&logo=next.js&logoColor=white)](https://nextjs.org/)
[![Groq](https://img.shields.io/badge/Powered%20by-Groq-f97316?style=flat-square)](https://groq.com/)
[![LLaMA](https://img.shields.io/badge/LLaMA-3.3%2070B-3b82f6?style=flat-square)](https://llama.meta.com/)
...
> 🎤 Transcribes live &nbsp;·&nbsp; ⚡ Generates typed suggestions every 30s &nbsp;·&nbsp; 💬 Streams full-context AI chat
...
**[→ meeting-assistant-sigma-roan.vercel.app](https://meeting-assistant-sigma-roan.vercel.app)**

No setup required. Bring your own Groq API key and start a meeting instantly.

---

## ✨ What It Does

MeetingMind transcribes your meeting in real time, detects the conversation's intent, and surfaces **3 typed, confidence-scored AI suggestions every 30 seconds** — questions to ask, talking points to raise, answers to give, and facts to check. Click any suggestion to open a streaming AI chat with full meeting context.
...
| ⚡ **Smart Suggestions** | 3 structurally diverse typed suggestions per batch, auto-refreshed |
| 🎯 **Intent Detection** | Auto-classifies as Interview / Sales / Technical / Brainstorming / Casual — adapts suggestions accordingly |
| 📊 **Confidence Scores** | Every suggestion carries a 0–100% confidence score, visualised as a progress bar |
| 💬 **Streaming Chat** | Click any suggestion → full streamed AI response with complete meeting context |
| 📤 **JSON Export** | One-click export of the full session: transcript + suggestions + chat history with timestamps |
...
A single combined call asking the model to "also detect intent" is unreliable — the model sometimes forgets or bleeds it into the JSON. Two focused calls are more predictable, and intent detection adds under **100ms** total latency.
...
│
├── components/
│   ├── Header.tsx                  # Record controls, intent badge, export
...
All settings are persisted in `localStorage` and editable live — no restart required:

| Setting | Default | Description |
|---|---|---|
| Suggestion Prompt | Built-in | Full prompt template with `{{intent}}` + `{{context}}` placeholders |
| Chat Prompt | Built-in | System prompt with `{{suggestion}}` + `{{full_context}}` placeholders |
| Context Window | 5 chunks | How many recent transcript chunks to include in suggestion calls |
...
- **Streaming chat** — first token surfaces in ~300ms
- **Bounded prompt size** — sliding window keeps token count constant regardless of meeting length
- **Audio size guard** — near-empty blobs are dropped before reaching Whisper
...
Typed suggestions enable instant visual scanning — at a glance you can see "there's an answer here and a question there." Free-form suggestions all look the same and slow decision-making in a live, time-pressured context.
...
&nbsp;
[![Live Demo](https://img.shields.io/badge/Live%20Demo-MeetingMind-38bdf8?style=flat-square&logo=vercel&logoColor=white)](https://meeting-assistant-sigma-roan.vercel.app)
